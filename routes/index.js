...
router.get("/addtocart/:productID", isLoggedIn, async (req, res) => {
  ...
  cartItem ? cartItem.quantity++ : user.cart.push({ product: req.params.productID });
  await user.save();
  res.redirect("/shop");
});
...
router.post("/removeFromCart/:productID", isLoggedIn, async (req, res) => {
  const user = await userModel.findOne({ email: req.user.email });
  const index = user.cart.findIndex(item => item.product.toString() === req.params.productID);
  if (index !== -1) {
    if (user.cart[index].quantity > 1) user.cart[index].quantity--;
...
router.post("/bargain/:productId", isLoggedIn, async (req, res) => {
  ...
  if (!product || isNaN(proposedPrice)) return res.status(400).send("Invalid product or price");
...
router.post("/bargain/:id/approve", isLoggedIn, async (req, res) => {
  ...
  buyer.cart.push({
    product: bargain.productId,
    quantity: 1,
    bargainedPrice: bargain.proposedPrice,
  });
  await buyer.save();
...
router.get("/pay-now/:userID", isLoggedIn, async (req, res) => {
  try {
    ...
    const buyer = await userModel.findById(req.params.userID).populate("cart.product");
    if (buyer.wallet < totalPrice) return res.status(400).send("Insufficient wallet balance");
    ...
    for (const item of buyer.cart) {
      ...
      const price = item.bargainedPrice || product.pPrice;
      const earnings = item.quantity * price;
...
// Admin
router.get("/admin", isLoggedIn, (req, res) => res.render("admin"));
...
router.get("/admin-products", isLoggedIn, async (req, res) => {
  try {
    ...
    const sales = await saleModel.find({ productId: { $in: productIds } })
      .populate("productId userId");

    const totalEarnings = sales.reduce((acc, sale) => acc + sale.totalEarnings, 0);
